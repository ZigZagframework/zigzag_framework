...
from model.train_model import TranModel
import string
import time
import os
import random
...
from preprocess.process_data import find_hard_examples
...
parser.add_argument('--learningRate', type=str, default='0.0012, 0.001, 0.0003, 0.0001',
                    help='')
...
def train_begin(model_path, train_times):
    ...
    while i < train_times:
        ...
        if not stop_flag and max_acc > 0.78 and max_f_score > 0.67:
            stop_flag = True
        if stop_flag and stop_step >= max_stop_step and i > 15:
            break
        if stop_step and max_f_score > f_score:
...
if __name__ == "__main__":
    ...
    serialNumber = f'epochTimes-{epochTimes}-learningRate-{learningRate}-modelKind-{modelKind}-predThreshold-{predThreshold}-is_find_hard-{str(is_find_hard)}'
    ...
    metricsFileName = f'epochTimes-{epochTimes}-learningRate-{learningRate}-modelKind-{modelKind}-predThreshold-{predThreshold}-is_find_hard-{str(is_find_hard)}-metrics.csv'
    ...
    # evaluation_with_predict(testDataSetPath, modelLast, metricsFile, predThreshold, 'model-3.1.h5')
